...
public class LoginActivity extends AppCompatActivity {
    ...
    private void initView() {
        ...
        ckRemberPass = (CheckBox) findViewById(R.id.ck_rember_pass);
    ...
    private void loadLoginParam() {
        ...
        if(read != null) {
            ...
            boolean isRember = read.getBoolean("is_rember", false);
            this.username.setText(username);
            this.password.setText(password);
            this.ckRemberPass.setChecked(isRember);
        }
    }
    ...
    public void onLoginClick() {
        ...
        UserController.userLogin(param, new Observer() {
            ...
            public void update(Observable observable, Object data) {
                ...
                if(result.isFlag()) {
                    SharedPreferences.Editor editor = getSharedPreferences(getResources().getString(R.string.app_shared_preferences), MODE_WORLD_WRITEABLE).edit();
                    if (ckRemberPass.isChecked()) {
                        //步骤2-2：将获取过来的值放入文件
                        editor.putString("u_password", strpassword);
                        editor.putString("u_username", strusername);
                        editor.putBoolean("is_rember", ckRemberPass.isChecked());
                    } else {
                        editor.putString("u_password", "");
                        editor.putString("u_username", String.valueOf(username));
                        editor.putBoolean("is_rember", ckRemberPass.isChecked());
                    }
                    editor.commit();
